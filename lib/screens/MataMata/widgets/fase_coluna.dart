...
class FaseColuna extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text(titulo, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey)),
        const SizedBox(height: 8),
        ...List.generate(jogos, (index) => _buildCard(context)),
      ],
    );
  }
      ...
      child: IntrinsicHeight(
        child: Stack(
          children: [
            Row(
              children: invertido
                  ? [_buildCampos(context), Container(width: 1, color: Theme.of(context).dividerColor), _buildTimes(context, alinharEsquerda: true)]
                  : [_buildTimes(context, alinharEsquerda: false), Container(width: 1, color: Theme.of(context).dividerColor), _buildCampos(context)],
            ),
            Positioned(
  ...
  Widget _buildTimes(BuildContext context, {required bool alinharEsquerda}) {
    return Expanded(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          _timeRow(context, '🇧🇷', 'Brasil', alinharEsquerda),
          _timeRow(context, '🇩🇪', 'Alemanha', alinharEsquerda),
        ],
      ),
    );
  }
